...
class milestone1(QMainWindow):
    def __init__(self):
        ...
        self.loadStateList()
        self.ui.stateList.currentTextChanged.connect(self.stateChanged) # calling the stateChanged value, self.stateChanged not calling the function
    ...
    def executeQuery(self,sql_str):
        try:
            conn = psycopg2.connect("dbname='milestone1db' user='danieloh' host='localhost' password='bullshit'")
        except:
            print('Unalbe to connect to the database!')
        cur = conn.cursor()
        cur.execute(sql_str)
        conn.commit()
        result = cur.fetchall()
    ...
    def stateChanged(self):
        ...
        if(self.ui.stateList.currentIndex()>=0):
           ...
           # for i in reversed(range(self.ui.businessTable.rowCount())): # removed because of error
             #   self.ui.businessTable.removeRow(i)

            sql_str = "SELECT name, city, state FROM business WHERE state ='" + state + "' ORDER BY name ;"
            try:
                results = self.executeQuery(sql_str)
                style = "::selection {""background-color: #f3f3f3; }"
                ...
                for row in results:
                    for colCount in range (0, len(results[0])):
                        self.ui.businessTable.setItem(currentRowCount,colCount,QTableWidgetItem(row[colCount])) #initialize QTableWidget constructor with data at row, col
                    currentRowCount += 1
            except:
                print("Query failed!")
    ...
    def getBusinessNames(self):
        ...
        try:
            results = self.executeQuery(sql_str)
            for row in results:
                self.ui.businesses.addItem(row10)
        except:
            print("Query failed!")

    def displayBusinessCity(self):
        businesses= self.ui.businessses.selectedItems()[0].text()
